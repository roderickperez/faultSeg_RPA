...
def _to_floats(seq):
    ...
    for v in seq:
        # CuPy 0-D?
        if isinstance(v, cp.ndarray) and v.ndim == 0:
            out.append(float(cp.asnumpy(v)))
        else:
            try:
                out.append(float(v))
            except Exception:
                # last resort—try NumPy coercion
                out.append(float(np.asarray(v).squeeze()))
    return out
...
def plot_dataset_stats(all_stats_data, base_out, image_dir, title_suffix="Dataset Statistics"):
    ...
    for split_name in ['full', 'train', 'validation']:
        ...
        strikes = _to_floats([f['strike'] for f in fault_params])
        dips = _to_floats([f['dip'] for f in fault_params])
        displacements = _to_floats([f['applied_disp_signed'] for f in fault_params])
        ...
        if strikes:
            if mask_mode == 1:
                ...
                num_bins = 18
                bins = np.linspace(0, 2*np.pi, num_bins + 1, dtype=np.float32)
