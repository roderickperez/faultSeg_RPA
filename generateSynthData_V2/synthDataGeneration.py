...
def generate_dataset(train_count, val_count, mask_mode=1):
    ...
    train_class_counts = np.zeros(3 if mask_mode==1 else 2, dtype=np.int64)
    val_class_counts   = np.zeros(3 if mask_mode==1 else 2, dtype=np.int64)
    train_sum_class_pct = np.zeros_like(train_class_counts, dtype=float)
    val_sum_class_pct   = np.zeros_like(val_class_counts, dtype=float)
    ...
    for i in tqdm(range(train_count), desc="Generating training cubes"):
        num_faults = train_fault_counts[i] if i < len(train_fault_counts) else np.random.randint(1, 9)
        # Step 1: Base horizontal reflectivity volume
        volume = np.zeros((NZ, NY, NX), dtype=float)
        trace = np.random.uniform(-1.0, 1.0, size=NZ)
        for z in range(NZ):
            volume[z, :, :] = trace[z]
        ...
        if apply_deformation:
            ...
            classic_polarity = 'up' if np.random.rand() < 0.5 else 'down'
            ...
            for _ in range(int(classic_num_bumps)):
                ...
                c_k = np.random.uniform(*cx_range)
                d_k = np.random.uniform(*cy_range)
        ...
        if apply_faulting:
            ...
            fault_mask = np.zeros((NZ, NY, NX), dtype=np.uint8)       # 2-voxel thick training mask
            fault_mask_display = np.zeros((NZ, NY, NX), dtype=np.uint8)  # 1-voxel break display
            fault_id_display   = np.zeros((NZ, NY, NX), dtype=np.int16)  # fault ID at break (for plane reconstruction)
            ...
            def sample_strike_deg(strike_range, mode='random'):
                ...
                if mode == 'random':
                    return np.random.uniform(*strike_range)
                # mode == 'two_sets'
                ...
                return (base_angle + np.random.uniform(-strike_two_set_spread, strike_two_set_spread)) % 360.0
            ...
            def sample_orientation():
                ...
                cos_dip = np.random.uniform(np.cos(dmax), np.cos(dmin))
            ...
            for fid, fault in enumerate(accepted_faults, start=1):
                ...
                new_mask = np.zeros_like(fault_mask, dtype=np.uint8)
                new_mask_disp = np.zeros_like(fault_mask_display, dtype=np.uint8)
                new_id_disp = np.zeros_like(fault_id_display, dtype=np.int16)
            ...
            fault_mask = np.zeros((NZ, NY, NX), dtype=np.uint8)

        # Step 5: Band-limit with wavelet and add noise
        peak_freq = np.random.uniform(*wavelet_peak_freq_range)
        # Create Ricker wavelet
        t = np.linspace(-1, 1, wavelet_length)
        pf = float(peak_freq)
        t2 = t**2; pi2f2t2 = (np.pi**2) * (pf**2) * t2
        ricker = (1 - 2*pi2f2t2) * np.exp(-pi2f2t2)
        ricker /= np.max(np.abs(ricker))
        # 1D convolution along depth (Z) axis using FFT (same length output)
        n = faulted_volume.shape[0]
        m = ricker.size
        L = int(2 ** np.ceil(np.log2(n + m - 1)))
        V = np.fft.rfft(faulted_volume, n=L, axis=0)
        W = np.fft.rfft(ricker, n=L)
        V *= W[:, None, None]
        conv_full = np.fft.irfft(V, n=L, axis=0)
        ...
        if apply_noise:
            if noise_type == 'gaussian':
                ...
                seismic += np.random.normal(0.0, noise_std, size=seismic.shape)
                noise_sigma_val = noise_std
            elif noise_type == 'uniform':
                rng = noise_intensity * (seismic.max() - seismic.min())
                seismic += np.random.uniform(-rng, rng, size=seismic.shape)
            elif noise_type == 'speckle':
                seismic *= np.random.normal(1.0, noise_intensity, size=seismic.shape)
            elif noise_type == 'salt_pepper':
                frac = noise_intensity
                total_vox = seismic.size
                n_noisy = int(frac * total_vox)
                if n_noisy > 0:
                    coords = np.unravel_index(np.random.choice(total_vox, size=n_noisy, replace=False), seismic.shape)
                    half = n_noisy // 2
    ...
    for j in tqdm(range(val_count), desc="Generating validation cubes"):
        num_faults = val_fault_counts[j] if j < len(val_fault_counts) else np.random.randint(1, 9)
        ...
        if apply_faulting:
            ...
            X2d, Y2d = np.meshgrid(np.arange(NX), np.arange(NY), indexing='xy')
            fault_mask = np.zeros((NZ, NY, NX), dtype=np.uint8)
            fault_mask_display = np.zeros((NZ, NY, NX), dtype=np.uint8)
            fault_id_display = np.zeros((NZ, NY, NX), dtype=np.int16)
            faulted_volume = volume.copy()
            accepted_faults = []
            attempts = 0
        ...
        peak_freq = np.random.uniform(*wavelet_peak_freq_range)
        t = np.linspace(-1, 1, wavelet_length)
        pf = float(peak_freq); t2 = t**2; pi2f2t2 = (np.pi**2)*(pf**2)*t2
        ricker = (1 - 2*pi2f2t2) * np.exp(-pi2f2t2)
        ricker /= np.max(np.abs(ricker))
        n = faulted_volume.shape[0]; m = ricker.size
        L = int(2 ** np.ceil(np.log2(n + m - 1)))
        V = np.fft.rfft(faulted_volume, n=L, axis=0)
        W = np.fft.rfft(ricker, n=L)[:, None, None]
        conv_full = np.fft.irfft(V * W, n=L, axis=0)
        start = (m-1)//2; end = start + n
